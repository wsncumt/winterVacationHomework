...
\newpage
\section{}
\num （\textbf{多选题}）关于\dm{Java}以下描述正确的有（~~\textcolor[rgb]{1.00,0.00,0.00}{C}、\textcolor[rgb]{1.00,0.00,0.00}{D}~~）。

A.\dm{Class}类是\dm{Object}类的超类\hfill  B.\dm{Object}类是一个\dm{final}类
...
\num （\textbf{多选题}）\dm{Java}中的集合类包括\dm{ArrayList}、\dm{LinkedList} 、\dm{HashMap}等类，下列关于集合类描述正确的是（~\textcolor[rgb]{1.00,0.00,0.00}{A}、\textcolor[rgb]{1.00,0.00,0.00}{B}、\textcolor[rgb]{1.00,0.00,0.00}{D}~）。
...
\begin{tips}
\dm{ArrayList}和\dm{LinkedList}均实现了\dm{List}接口。

\dm{ArrayList}在内存中基于数组实现的，为寻秩访问，访问速度快，但增加和删除元素慢；\dm{LinkedList}基于链表存放，为寻地址访问，访问速度慢，添加和删除元素略优。

\dm{HashMap}实现\dm{Map}接口，允许任何类型的键和值对象，允许将\dm{null}用作键或值
\end{tips}

\num （\textbf{多选题}）关于\dm{equals}和\dm{hashCode}描述正确的是 （~\textcolor[rgb]{1.00,0.00,0.00}{A}、\textcolor[rgb]{1.00,0.00,0.00}{B}、\textcolor[rgb]{1.00,0.00,0.00}{C}~）。
...
2.每种元素$a_i$只有两种状态，在子集中和不在子集中，用1表示在子集中，用0表示不在子集中。

3.每个子集可以对应一个长度为n的0/1序列，第i位表示$a_i$是否在子集中。
...
\newpage
\section{}
\num （\textbf{多选题}）下面有关\dm{java}的\dm{instanceof}、 \dm{?}、\dm{&} 、\dm{&&}说法正确的有（\textcolor[rgb]{1.00,0.00,0.00}{A}、\textcolor[rgb]{1.00,0.00,0.00}{B}、\textcolor[rgb]{1.00,0.00,0.00}{C}、\textcolor[rgb]{1.00,0.00,0.00}{D}）。
...
\num （\textbf{多选题}）下面哪个语句是创建数组的正确语句？（\textcolor[rgb]{1.00,0.00,0.00}{A}、\textcolor[rgb]{1.00,0.00,0.00}{B}、\textcolor[rgb]{1.00,0.00,0.00}{D}、\textcolor[rgb]{1.00,0.00,0.00}{E}）。
...
\begin{tips}
多维数组行可以省略，列不能省略。等号右边的第一个\dm{[]}是列，第二个\dm{[]}是行。
\end{tips}

\num 下列类在多重\dm{catch}中同时出现时，哪一个异常类应最后一个列出（~~~\textcolor[rgb]{1.00,0.00,0.00}{C}~~~）。
...
3.当队列不为空时，首先记录当前队列的大小\dm{size}

4.对队列（当前层）进行遍历：
\begin{itemize}
  \item 取出队首元素\dm{node}。
  \item 如果该元素\dm{node}的左孩子不为空，左孩子入队。
  \item 如果该元素\dm{node}的右孩子不为空，右孩子入队。
\end{itemize}

5.当前层遍历完后，\dm{node}为当前层的最右侧元素，将该元素的值入结果。
...
2.\dm{match[i][j]}表示当前\dm{p}的前\dm{i}个字符和\dm{s}的前\dm{j}个字符是否匹配

3.状态转移：
\begin{itemize}
  \item 如果\dm{p[i - 1] == s[j - 1]}或\dm{p[i - 1] == '?'}，表明当前字符匹配，\dm{match[i][j]}可以从\dm{match[i - 1][j - 1]}转移而来。
  \item 如果\dm{p[i - 1] == '*'}，这个位置可以匹配0到若干个字符，\dm{match[i][j]}从其前一个状态转移来。其前一个状态包括\dm{match[i - 1][j]}（表示模式串的前i-1个字符与字符串的前j个字符的匹配状态），\dm{match[i][j - 1]}（表示模式串的前i个字符与字符串的前j-1个字符的匹配状态）
\end{itemize}

4.初始条件：\dm{match[0][0]}表示两个空串匹配。
